...
class EkfReflectometryProcessor:
    """Online multi-arc EKF over RH and per-arc amplitude/phase states.

    The implementation follows the MATLAB EKF_GNSSIR high-accuracy model in
    no-tide mode: one shared reflector height state RH, local [A, phi] states
    per initialized arc, and full covariance across arcs until each arc ends.
    """
    ...
    def build_products(self, outputs: list[EkfOutput] | None = None) -> list[ProductResult]:
        ...
        for output in selected_outputs:
            ...
            metadata = {
                ...
                "active_satellite_arc_count": output.active_arc_count,
        ...
        for observation in sorted(observations, key=lambda item: item.satellite_system_key):
            ...
            if not arc_state.initialized:
                ...
                if self.rh_initialized:
                    self._try_initialize_arc(arc_state)
    ...
    def _new_arc_state(self, observation: ObservationRecord) -> _ArcState:
        ...
        arc_state = _ArcState(key=key, wavelength_m=wavelength)
    ...
    def _try_initialize_arc(self, arc_state: _ArcState) -> None:
        if not self.rh_initialized:
            return
        ...
        if len(samples) < int(self.config.init_min_samples):
            return
        rh_now = max(abs(float(self.rh)), 0.5)
        sin_values = np.asarray([item[2] for item in samples], dtype=float)
        span_needed = arc_state.wavelength_m / rh_now
        if float(np.max(sin_values) - np.min(sin_values)) < span_needed:
            return
        y = np.asarray([item[1] for item in samples], dtype=float)
        A0, phi0, sigma = _fit_amplitude_phase(y, sin_values, self.rh, arc_state.wavelength_m)
        if not np.isfinite(A0) or not np.isfinite(phi0):
            return
        self._insert_arc(arc_state, float(A0), float(phi0))
    ...
    def _try_initialize_reflector_height(self) -> None:
        initialization = _estimate_rh_lsp_from_arcs(
            ...
            grid_size=int(self.ir_config.frequency_grid_size),
        )
    ...
    def _insert_arc(self, arc_state: _ArcState, amplitude: float, phase: float) -> None:
        if arc_state.initialized:
            return
        n_old = len(self.state_arcs)
        n_new = n_old + 1
        old_x = self._state_vector()
        new_x = np.zeros(1 + 2 * n_new, dtype=float)
        new_x[0] = old_x[0]
        if n_old:
            new_x[1 : 1 + n_old] = old_x[1 : 1 + n_old]
            new_x[1 + n_new : 1 + n_new + n_old] = old_x[1 + n_old :]
        new_x[1 + n_old] = amplitude
        new_x[1 + n_new + n_old] = phase

        P_new = np.zeros((1 + 2 * n_new, 1 + 2 * n_new), dtype=float)
        if n_old:
            old_rows = [0] + list(range(1, 1 + n_old)) + list(range(1 + n_old, 1 + 2 * n_old))
            new_rows = [0] + list(range(1, 1 + n_old)) + list(range(1 + n_new, 1 + n_new + n_old))
            P_new[np.ix_(new_rows, new_rows)] = self.P[np.ix_(old_rows, old_rows)]
        else:
            P_new[0, 0] = self.P[0, 0]
        local_var = float(self.config.local_state_variance)
        P_new[1 + n_old, 1 + n_old] = local_var
        P_new[1 + n_new + n_old, 1 + n_new + n_old] = local_var

        self.state_arcs.append(arc_state)
        arc_state.initialized = True
        arc_state.local_index = n_old
        arc_state.amplitude = amplitude
        arc_state.phase = phase
    ...
    def _remove_arc(self, key: tuple[str, str, str]) -> None:
        ...
        if arc_state.initialized and arc_state in self.state_arcs:
            ...
            n_old = len(self.state_arcs)
            n_new = n_old - 1
            rows = [0] + [1 + index for index in keep_local] + [1 + n_old + index for index in keep_local]
            new_x = old_x[rows]
    ...
    def _ekf_step(self, timestamp: datetime, valid_epoch: list[tuple[_ArcState, float, float]]) -> None:
        ...
        if n_local:
            a_rows = np.arange(1, 1 + n_local)
            P_pred[a_rows, a_rows] += float(self.config.q_amplitude)

        measurements: list[tuple[float, float, _ArcState]] = []
        for arc_state, z_value, sin_elevation in valid_epoch:
            if arc_state.local_index is None or arc_state not in self.state_arcs:
                continue
            measurements.append((z_value, sin_elevation, arc_state))

        active_satellite_arc_count = _satellite_arc_count(
            arc_state for _z, _sin_elevation, arc_state in measurements
        )
        if active_satellite_arc_count < int(self.config.min_active_arcs):
            self._set_state_vector(x_pred)
            self.P = P_pred
            return

        m = len(measurements)
        H = np.zeros((m, 1 + 2 * m), dtype=float)
        local_indices = [int(arc_state.local_index) for _z, _sin_elevation, arc_state in measurements]
        active_rows = [0]
        active_rows.extend(1 + local_index for local_index in local_indices)
        active_rows.extend(1 + n_local + local_index for local_index in local_indices)
        z = np.zeros(m, dtype=float)
        h = np.zeros(m, dtype=float)
        for obs_index, (z_value, sin_elevation, arc_state) in enumerate(measurements):
            local_index = int(arc_state.local_index)
            alpha = (4.0 * math.pi / arc_state.wavelength_m) * sin_elevation
            amplitude = x_pred[1 + local_index]
            phase = x_pred[1 + n_local + local_index]
            varphi = alpha * x_pred[0] + phase
            z[obs_index] = z_value
            h[obs_index] = amplitude * math.sin(varphi)
            H[obs_index, 0] = amplitude * math.cos(varphi) * alpha
            H[obs_index, 1 + obs_index] = math.sin(varphi)
            H[obs_index, 1 + m + obs_index] = amplitude * math.cos(varphi)

        active_rows_array = np.asarray(active_rows, dtype=int)
        P_active = P_pred[np.ix_(active_rows_array, active_rows_array)]
        innovation = z - h
        S = H @ P_active @ H.T + float(self.config.measurement_variance) * np.eye(m, dtype=float)
    ...
    def _state_vector(self) -> np.ndarray:
        n_local = len(self.state_arcs)
        x = np.zeros(1 + 2 * n_local, dtype=float)
        x[0] = self.rh
        for index, arc_state in enumerate(self.state_arcs):
            x[1 + index] = arc_state.amplitude
            x[1 + n_local + index] = arc_state.phase
        return x
    ...
    def _set_state_vector(self, x: np.ndarray) -> None:
        ...
        n_local = len(self.state_arcs)
        for index, arc_state in enumerate(self.state_arcs):
            arc_state.amplitude = float(x[1 + index])
            arc_state.phase = float(x[1 + n_local + index])
...
def _fit_amplitude_phase(
    ...
    theta = (4.0 * math.pi * float(rh_m) / float(wavelength_m)) * sin_elevation
    ...
    fitted = G @ coef
    sigma = float(np.sqrt(np.mean(np.square(residual - fitted)))) if residual.size else float("nan")
...
def _estimate_rh_lsp_from_arcs(
    ...
    *,
    ...
    candidates_by_satellite: dict[tuple[str, str], list[float]] = {}
    satellite_order: list[tuple[str, str]] = []
    rh_grid = np.linspace(min_height_m, max_height_m, max(grid_size, 32))
    ...
    for arc_state in ready_arcs:
        ...
        estimate = _estimate_arc_rh_grid(y, s, arc_state.wavelength_m, rh_grid)
        if estimate is None:
            continue
        satellite_key = _satellite_arc_key(arc_state)
        if not any(satellite_key):
            continue
        if satellite_key not in candidates_by_satellite:
            if len(candidates_by_satellite) >= max_arcs:
                continue
            candidates_by_satellite[satellite_key] = []
            satellite_order.append(satellite_key)
        candidates_by_satellite[satellite_key].append(float(estimate))

    if not candidates_by_satellite:
        return None
    arc_count = len(candidates_by_satellite)
    if arc_count < min_arcs:
        return None

    estimates = [
        float(np.median(candidates_by_satellite[satellite_key]))
        for satellite_key in satellite_order
    ]
    return RhInitialization(
        reflector_height_m=float(np.mean(estimates)),
        arc_count=arc_count,
...
def _estimate_arc_rh_grid(
    ...
    for rh_value in rh_grid:
        theta = (4.0 * math.pi * float(rh_value) / float(wavelength_m)) * sin_elevation
        design = np.column_stack((np.sin(theta), np.cos(theta)))
        coef, *_ = np.linalg.lstsq(design, y, rcond=None)
        fitted = design @ coef
        rss = float(np.sum(np.square(y - fitted)))
        if rss < best_rss:
            best_rss = rss
