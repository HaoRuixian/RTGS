...
def test_merge_rinex_daily_files_resamples_on_gps_time_scale(tmp_path):
    source_dir = tmp_path / "hourly_gps_15s"
    source_dir.mkdir()
    ...
    out_dir = tmp_path / "daily_gps_15s"
    result = merge_rinex_daily_files(
        [source_dir],
        out_dir,
        marker_name="Mosaic-X5",
    ...
    text = result.output_files[0].read_text(encoding="utf-8")
    assert "> 2026 03 24 00 00 30.0000000" in text
    assert "> 2026 03 24 00 00 45.0000000" in text
    assert "> 2026 03 24 00 00 18.0000000" not in text
    assert "> 2026 03 24 00 00 33.0000000" not in text
