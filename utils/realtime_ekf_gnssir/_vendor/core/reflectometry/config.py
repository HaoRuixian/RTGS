...
ir:
  ...
  ekf:
    ...
    robust_min_sigma: 0.05
    robust_huber_k: 3.5
    robust_min_weight: 0.25
    initial_rh_m: 8.0
    ...
    rh_init_cycles: 2.0
    rh_init_average: "mean"
    rh_search_step_m: 0.025
    phase_center_offsets_m: []
...
class EkfConfig:
    ...
    robust_min_sigma: float = 0.05
    robust_huber_k: float = 3.5
    ...
    rh_init_cycles: float = 2.0
    rh_init_average: str = "mean"
    rh_search_step_m: float = 0.025
    phase_center_offsets_m: list[dict[str, Any]] = field(default_factory=list)
    ...
    def __post_init__(self) -> None:
        ...
        if self.q_varphi < 0:
            raise ValueError("ir.ekf.q_varphi must be >= 0")
        if self.q_amplitude < 0:
            raise ValueError("ir.ekf.q_amplitude must be >= 0")
        if self.q_phase < 0:
            raise ValueError("ir.ekf.q_phase must be >= 0")
        if self.measurement_variance <= 0:
            raise ValueError("ir.ekf.measurement_variance must be > 0")
        if self.robust_min_sigma <= 0:
            raise ValueError("ir.ekf.robust_min_sigma must be > 0")
        if self.robust_huber_k <= 0:
            raise ValueError("ir.ekf.robust_huber_k must be > 0")
        if not (0 < self.robust_min_weight <= 1):
            raise ValueError("ir.ekf.robust_min_weight must be in (0, 1]")
        ...
        if self.rh_init_cycles <= 0:
            raise ValueError("ir.ekf.rh_init_cycles must be > 0")
        if self.rh_search_step_m <= 0:
            raise ValueError("ir.ekf.rh_search_step_m must be > 0")
