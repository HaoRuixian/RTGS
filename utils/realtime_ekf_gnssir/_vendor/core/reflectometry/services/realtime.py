...
class RealtimeProcessor:
    ...
    def __init__(
        ...
        self.ekf_processor = (
            ...
            if config.ir.estimation_mode == "ekf"
            else None
        )
    ...
    def ingest(
        ...
        *,
        ...
        if self.ekf_processor is not None:
            return self._ingest_ekf(observations, reference_time=reference_time)
    ...
    def snapshot(
        ...
        *,
        ...
        if self.ekf_processor is not None:
            return self._snapshot_ekf(reference_time=reference_time)
    ...
    def _ingest_ekf(
        ...
        *,
        ...
        assert self.ekf_processor is not None
        self.ekf_processor.ingest(observations)
        return self._snapshot_ekf(reference_time=reference_time)

    def _snapshot_ekf(self, *, reference_time: datetime | None = None) -> ProcessingRunResult:
        assert self.ekf_processor is not None
        products = self.ekf_processor.build_products()
        snapshot_time = reference_time or (products[-1].timestamp if products else datetime.utcnow())
        if products:
        ...
        self.last_result = ProcessingRunResult(
            ...
            metadata={
                "mode": "realtime",
                "estimation_mode": "ekf",
                "ekf_output_count": len(self.ekf_processor.snapshot_outputs()),
            },
        )
        return self.last_result
    ...
    def reset(self) -> None:
        ...
        self.buffers.clear()
        if self.ekf_processor is not None:
            self.ekf_processor = EkfReflectometryProcessor(
                self.config.ir,
