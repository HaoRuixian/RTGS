...
except Exception:  # pragma: no cover - optional spectral dependency
    find_peaks = None
    lombscargle = None
    peak_widths = None
...
def compute_lomb_scargle(
    ...
    if lombscargle is None:
        raise RuntimeError("scipy is required for Lomb-Scargle spectrum analysis.")
...
def detect_peaks(
    ...
    if find_peaks is None or peak_widths is None:
        raise RuntimeError("scipy is required for spectral peak detection.")
