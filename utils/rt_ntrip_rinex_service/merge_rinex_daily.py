...
from .rinex_loader import RinexObservationReader, read_rinex_observation_header
from .gnss_time import GNSSTime
from .split_rinex_hourly import (
...
def _day_bucket(epoch_time: datetime, time_system: str) -> datetime:
    output_time = _convert_output_time(epoch_time, time_system)
    output_day_start = output_time.replace(hour=0, minute=0, second=0, microsecond=0)
    return _convert_time_to_utc(output_day_start, time_system)
    ...
    *,
    ...
    try:
        for epoch in chain(buffered_epochs, epoch_iter):
            ...
            bucket = _day_bucket(selected_time, time_system)
