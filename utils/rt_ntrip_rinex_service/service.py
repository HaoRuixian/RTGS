...
from .data_models import EpochObservation
from .gnss_time import GNSSTime
from .mixed_gnss_reader import MixedGNSSReader
from .rinex3_writer import RINEX3Writer

try:
    from .pyrtcm_compat import patch_pyrtcm_glonass_g3
...
class RinexStationConfig:
    ...
    header_refresh_seconds: int = 60
    fsync_interval_seconds: int = 0
...
ObsTypesPersistFn = Callable[[str, Dict[str, List[str]]], None]
ApproxPositionPersistFn = Callable[[str, List[float]], None]
...
def _build_station_config(raw_station: dict, *, config_dir: Path) -> RTStationConfig:
    ...
    configured_time_system = str(rinex_block.get("time_system", "GPS")).strip().upper() or "GPS"
    if configured_time_system not in SUPPORTED_TIME_SYSTEMS:
        raise ValueError(
    ...
    rinex = RinexStationConfig(
        ...
        header_refresh_seconds=_parse_int(
            rinex_block.get("header_refresh_seconds", 60),
            field_name=f"Station {name}: rinex.header_refresh_seconds",
            min_value=1,
        ),
        fsync_interval_seconds=int(
            _parse_float(
                rinex_block.get("fsync_interval_seconds", 0),
                field_name=f"Station {name}: rinex.fsync_interval_seconds",
                min_value=0.0,
            )
        ),
    )
...
def _create_rtcm_handler() -> RTCMHandler:
    from .rtcm_handler import RTCMHandler
...
class RTNtripRinexStation(threading.Thread):
    ...
    def __init__(
        ...
        *,
        ...
        obs_types_persist_fn: Optional[ObsTypesPersistFn] = None,
        approx_position_persist_fn: Optional[ApproxPositionPersistFn] = None,
        ...
        self.obs_types_persist_fn = obs_types_persist_fn
        self.approx_position_persist_fn = approx_position_persist_fn
        ...
        self._ignored_new_obs_type_codes: set[str] = set()
        self._total_bytes = 0
        self._epochs_written = 0
        self._last_message_time: Optional[datetime] = None
        self._last_epoch_time: Optional[datetime] = None
        self._last_error: str = ""
        self._active_file_path: Optional[str] = None
        self._last_persisted_approx_position: Optional[List[float]] = None
    ...
    def snapshot(self) -> dict:
        with self.status_lock:
            return {
                ...
                "alive": self.is_alive(),
                "mountpoint": self.station.ntrip.mountpoint,
                "host": self.station.ntrip.host,
                "active_file": self._active_file_path,
                "epochs_written": self._epochs_written,
                "bytes_received": self._total_bytes,
                "last_message_time": self._last_message_time.isoformat() if self._last_message_time else None,
                "last_epoch_time": self._last_epoch_time.isoformat() if self._last_epoch_time else None,
                "last_error": self._last_error,
                "sample_interval_seconds": self._resolved_sample_interval_seconds,
                "sys_obs_types": dict(self.station.rinex.sys_obs_types),
                "approx_position": list(self.station.rinex.approx_position or []),
            }

    def update_runtime_config(self, station: RTStationConfig) -> None:
        """Apply config fields that are safe to change without reconnecting."""
        with self.status_lock:
            self.station.rinex.sys_obs_types = _normalize_sys_obs_types(station.rinex.sys_obs_types)
            self.station.rinex.auto_detect_obs_types = bool(station.rinex.auto_detect_obs_types)
            self.station.rinex.approx_position = station.rinex.approx_position
            self.station.rinex.header_refresh_seconds = int(station.rinex.header_refresh_seconds)
            self.station.rinex.fsync_interval_seconds = int(station.rinex.fsync_interval_seconds)
    ...
    def _year_doy_dir(self, output_time: datetime) -> Path:
        return self._year_dir(output_time) / output_time.strftime("%j")
    ...
    def _write_epoch(self, epoch_data: EpochObservation) -> None:
        ...
        with self.status_lock:
            self._epochs_written += 1
            self._last_epoch_time = aligned_epoch_time
        self._commit_active_writer()

    def _commit_active_writer(self) -> None:
        writer = self._active_writer
        if writer is None or writer.file_handle is None:
            return

        now_monotonic = time.monotonic()
        header_refresh_seconds = max(1, int(self.station.rinex.header_refresh_seconds))
        if now_monotonic - self._last_header_refresh_monotonic >= header_refresh_seconds:
            try:
                writer._refresh_header()
                self._last_header_refresh_monotonic = now_monotonic
            except Exception as exc:
                self.log(f"Header refresh failed for {Path(writer.filename).name}: {exc}")

        fsync_seconds = int(self.station.rinex.fsync_interval_seconds)
        if fsync_seconds > 0 and now_monotonic - self._last_fsync_monotonic >= fsync_seconds:
            try:
                writer.file_handle.flush()
                os.fsync(writer.file_handle.fileno())
                self._last_fsync_monotonic = now_monotonic
            except Exception as exc:
                self.log(f"Durable flush failed for {Path(writer.filename).name}: {exc}")
    ...
    def _build_sys_obs_types(self, satellites: dict) -> Dict[str, List[str]]:
        ...
        detected = detected or _copy_default_sys_obs_types()
        self._persist_detected_sys_obs_types(detected)
        return _normalize_sys_obs_types(self.station.rinex.sys_obs_types) or detected

    def _persist_detected_sys_obs_types(self, detected: Dict[str, List[str]]) -> None:
        normalized = _normalize_sys_obs_types(detected)
        if not normalized:
            return

        self.station.rinex.sys_obs_types = normalized
        self.station.rinex.auto_detect_obs_types = False
        if self.obs_types_persist_fn is None:
            self.log("Observation types detected; no config writer is attached, using in-memory values")
            return

        try:
            self.obs_types_persist_fn(self.station.name, normalized)
            self.log("Observation types detected and written to config")
        except Exception as exc:
            self.log(f"Observation type config update failed: {exc}")
    ...
    def _needs_obs_type_rollover(self, satellites: dict) -> bool:
        ...
        for system, codes in epoch_sys_obs_types.items():
            ...
            new_codes = sorted(code for code in codes if code not in current_codes)
            if new_codes:
                token = f"{system}:{','.join(new_codes)}"
                if token not in self._ignored_new_obs_type_codes:
                    self._ignored_new_obs_type_codes.add(token)
                    self.log(
                        f"Ignoring new observation type(s) not present in config for {system}: {', '.join(new_codes)}"
                    )
        return False
    ...
    def _resolve_approx_position(self) -> Optional[List[float]]:
        ...
        if position:
            ...
            if parsed is not None:
                self._persist_approx_position(parsed)
                return parsed
    ...
    @staticmethod
    def _positions_close(first: Optional[List[float]], second: Optional[List[float]], tolerance: float = 0.0001) -> bool:
        if first is None or second is None or len(first) < 3 or len(second) < 3:
            return False
        return all(abs(float(a) - float(b)) <= tolerance for a, b in zip(first[:3], second[:3]))

    def _persist_approx_position(self, approx_position: List[float]) -> None:
        if self._positions_close(self._last_persisted_approx_position, approx_position):
            return
        if self._positions_close(self.station.rinex.approx_position, approx_position):
            self._last_persisted_approx_position = list(approx_position[:3])
            return

        self.station.rinex.approx_position = list(approx_position[:3])
        self._last_persisted_approx_position = list(approx_position[:3])
        if self.approx_position_persist_fn is None:
            self.log("Approx position detected; no config writer is attached, using in-memory value")
            return

        try:
            self.approx_position_persist_fn(self.station.name, list(approx_position[:3]))
            self.log("Approx position detected and written to config")
        except Exception as exc:
            self.log(f"Approx position config update failed: {exc}")
    ...
    def _open_writer(self, aligned_epoch_time: datetime, satellites: dict) -> None:
        ...
        self._last_header_refresh_monotonic = time.monotonic()
        self._last_fsync_monotonic = self._last_header_refresh_monotonic
        with self.status_lock:
            self._active_file_path = str(Path(writer.filename).resolve())
    ...
    def _close_writer(self) -> None:
        ...
        writer.close()
        with self.status_lock:
            if self._active_file_path == str(file_path.resolve()):
    ...
    def _maybe_merge_daily_file(self, file_path: Path, file_start_time: Optional[datetime]) -> None:
        ...
        day_start = file_start_time.replace(hour=0, minute=0, second=0, microsecond=0)
        ...
        day_is_complete_by_time = day_start < current_day_start
        day_is_complete_by_count = len(source_files) >= expected_count
        if not source_files or not (day_is_complete_by_time or day_is_complete_by_count):
            return

        output_dir = day_dir.parent
        if self._daily_merge_output_is_fresh(output_dir, source_files, file_start_time):
            return
    ...
    def _daily_merge_output_is_fresh(
        self,
        output_dir: Path,
        ...
        if not source_files:
            return True
        rinex_cfg = self.station.rinex
        pattern = f"*_R_{file_start_time.strftime('%Y%j')}0000_01D_*_{rinex_cfg.datatype}.rnx"
...
class MultiStationRTRinexService:
    ...
    def __init__(
        ...
        *,
        ...
        self._obs_types_persist_fn = obs_types_persist_fn
        self._approx_position_persist_fn = approx_position_persist_fn
        self.workers = [
    ...
    def _build_worker(self, station: RTStationConfig) -> RTNtripRinexStation:
        ...
        return RTNtripRinexStation(
            station,
            log_fn=self.log_fn,
            obs_types_persist_fn=self._obs_types_persist_fn,
            approx_position_persist_fn=self._approx_position_persist_fn,
        )
